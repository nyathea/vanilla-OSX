...
static char
    pk_cookie[256]; // TODO: No actual idea how long a Polkit cookie usually is

G_DEFINE_TYPE(VanillaPolkitListener, vanilla_polkit_listener,
              POLKIT_AGENT_TYPE_LISTENER)

static void try_again();

static void on_request(PolkitAgentSession *s, const gchar *prompt,
                       gboolean echo_on, gpointer user_data) {
  vpilog("Polkit authentication requested\n");
  pk_session = s;
}

static void on_show_info(PolkitAgentSession *s, const gchar *text,
                         gpointer user_data) {
  vpilog("Info\n");
  if (text)
    vpilog("%s\n", text);
...
static void on_show_error(PolkitAgentSession *s, const gchar *text,
                          gpointer user_data) {
  vpilog("Error\n");
  if (text)
    vpilog("Error: %s\n", text);
...
static void try_again() {
  ...
  PolkitAgentSession *session =
      polkit_agent_session_new(pk_identity, pk_cookie);
...
static void vanilla_polkit_initiate_authentication(
...
static gboolean vanilla_polkit_initiate_authentication_finish(
...
static void vanilla_polkit_listener_finalize(GObject *object) {
  ...
  g_return_if_fail(
      G_TYPE_CHECK_INSTANCE_TYPE((object), vanilla_polkit_listener_get_type()));
  G_OBJECT_CLASS(vanilla_polkit_listener_parent_class)->finalize(object);
}

static void
vanilla_polkit_listener_class_init(VanillaPolkitListenerClass *self) {
  ...
  lklass->initiate_authentication_finish =
      vanilla_polkit_initiate_authentication_finish;
}
...
int vpi_start_epilog() {
  ...
  while (read_count < sizeof(ready_buf)) {
    ssize_t this_read =
        read(err_pipes[0], ready_buf + read_count, sizeof(ready_buf));
    if (this_read > 0) {
  ...
  if (!memcmp(ready_buf, "READY", 5)) {
    ...
    pthread_create(&pipe_log_thread, 0, vpi_pipe_log_thread,
                   (void *)(intptr_t)pipe_err);

    pipe_pid = potential_pipe_pid;
  } else {
    vpilog("GOT INVALID SIGNAL: %.*s\n", sizeof(ready_buf), ready_buf);

    // Kill seems to break a lot of things so I guess we'll just leave it
    // orphaned kill(pipe_pid, SIGKILL);
    pipe_pid = -1;
    close(in_pipes[1]);
    close(err_pipes[0]);
  }
...
int vpi_start_pipe() {
  ...
  // Get parent pid (allows us to check if parent was terminated immediately
  // after fork)
  ...
  if (pid == 0) {
    ...
    // See if parent pid is still the same. If not, it must have been
    // terminated, so we will exit too.
    ...
    if (_NSGetExecutablePath(exe, &exe_size) != 0) {
      vpilog("Failed to get executable path\n");
      _exit(1);
    }
#else
    ...
    r = execlp(pkexec, pkexec, exe, "-local", vpi_config.wireless_interface,
               (const char *)0);
#else
    r = execlp(exe, exe, "-local", vpi_config.wireless_interface,
               (const char *)0);
#endif
